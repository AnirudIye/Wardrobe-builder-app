...
2. **Quota flow is generalized across three independently-metered kinds.** `/recommendations/today` is free and unlimited (never add `quota.enforce` to it) - everything else is metered: `"buy-next"`, `"dresser-ai"`, and `"tryon"`, each with its own limit and window: `"buy-next"` is metered **per day** (`settings.free_daily_recommendation_limit`, `days=1`), while `"dresser-ai"` and `"tryon"` are per trailing week (`free_weekly_chat_limit` / `free_weekly_tryon_limit`, default `days=7`). Call `quota.enforce(db, user, kind, limit, days=...)` **before any paid API call** (raises HTTP 402 for free users over that kind's limit → the SPA routes to the upgrade screen), then `quota.record(db, user, kind)` **after** successfully serving - a failed AI call (e.g. TryOn generation returning `None`) must NOT call `record`, since the user got nothing for it. Usage = count of `RecommendationEvent` rows with that `kind` in its trailing window (`days` param); the table is the single source of truth (no counter column). `User.is_paid` (property: `plan == "paid"`) bypasses every kind. `BillingStatus` (`routers/billing.py`) reports remaining/limit per kind - adding a fourth metered feature means adding its own limit setting, its own `quota.remaining(...)` call in `billing_status`, and its own `BillingStatus` fields (additive, non-breaking). See `services/quota.py` and `routers/recommendations.py`/`dresser_ai.py`/`tryon.py`.
...
`POST /tryon` takes a multipart photo plus **either** `garment_id` (an owned wardrobe item - ownership checked via `wardrobe._get_owned_garment`; its bytes are read straight from the storage backend via `get_storage().read`, never over HTTP) **or** `image_url` (an external product image, e.g. a Buy Next thumbnail, fetched via `images.download_image_bytes` - shared with `wardrobe.py`'s "add from web", size-capped and SSRF-guarded: every URL and redirect hop must resolve to public addresses only). The bytes are sent to Gemini (`tryon.generate_tryon`, model `settings.google_image_model`) alongside the user's photo. Nothing is persisted - the result comes back as `image_base64` in the JSON response and is rendered directly via a `data:image/png;base64,...` URI on the frontend. A failed generation (no key / API error) returns HTTP 503 and does **not** consume the user's weekly quota (see quota pattern #2 above) - only a successful try-on counts.
...
Garment images go through the `StorageBackend` ABC (`save`/`url`/`read`/`delete` keyed by opaque relative keys). `STORAGE_BACKEND` selects the implementation: `local` (default - filesystem under `MEDIA_DIR`, served at `/media`, which is only mounted for this backend) or `s3` (`storage/s3.py`, any S3-compatible store incl. Cloudflare R2/Filebase; lazy boto3 import; objects served from `S3_PUBLIC_BASE_URL`, or - with `S3_PRESIGN=true` - via 7-day presigned GET URLs for private buckets, which is how production runs on Filebase's free plan). Media keys are unguessable uuid4. Callers store only the key.

### Database
Dev is SQLite and tables auto-create on startup via `Base.metadata.create_all` - **in dev only** (gated on `ENVIRONMENT != production`). **Alembic is wired** (`backend/alembic/`, url comes from the app's `DATABASE_URL`): production schema is an explicit `alembic upgrade head` release step, never run at app startup. Schema changes now mean a new autogenerated revision, not deleting `wardrobe.db`. Managed Postgres works out of the box (`psycopg3`; the engine adds `pool_pre_ping` for Neon-style autosuspend - see `.env.example` for URL forms and the Supabase pooler caveat). Tests use in-memory SQLite per test (`create_all`, Alembic never enters the test path), and `tests/conftest.py` blanks **every** external API key and disables the auth rate limiter before app import, so the suite never hits the network or spends quota - a test exercising an AI path must `monkeypatch` the service (see `test_llm.py`, `test_vision.py`).

> **Migration caveat - adopting a pre-Alembic database:** run `alembic stamp head` (tables already exist), and if it predates email confirmation, first run `UPDATE users SET email_verified = true` - login hard-blocks (403) unverified users, so skipping the backfill locks out every existing account. The baseline migration encodes this rule (see its docstring).

### Security posture
`ENVIRONMENT=production` flips the hardening: startup **fails fast** on a placeholder/short `JWT_SECRET` or missing `CORS_ORIGINS` (validator in `config.py`), `/docs`+`/redoc`+`/openapi.json` are disabled, and HSTS is added by `core/middleware.py`'s `SecurityHeadersMiddleware` (nosniff, Referrer-Policy, X-Frame-Options, Permissions-Policy - camera stays allowed for TryOn's getUserMedia). Auth endpoints are rate-limited per IP (`core/ratelimit.py`, in-process sliding window; per-process limitation documented), and login additionally has a per-account lockout (`core/lockout.py`: `LOGIN_LOCKOUT_ATTEMPTS` failures in `LOGIN_LOCKOUT_MINUTES` → 429 until the window rolls or a password reset is redeemed; shares the `RATE_LIMIT_ENABLED` switch). Login failures use one generic message - never reveal which of email/password was wrong. Remote image fetches are SSRF-guarded (`services/images.py`: public-IP-only DNS check on every redirect hop). The SPA ships a build-time CSP meta (`vite.config.ts` - `script-src 'self'`, no inline scripts; dev build stays CSP-free for the React preamble). **JWT stance:** Bearer token in localStorage is deliberate - no cookies means no CSRF surface, and the CSP is the XSS mitigation; an httpOnly-cookie + refresh-token flow is future work, not a quick swap. See `docs/deploy.md` for the deploy-time checklist (TLS at the platform, `--proxy-headers`, real CSP header with `frame-ancestors`).
...
Secrets live in `backend/.env` (git-ignored; template in `.env.example`). All keys are optional: **either** `ANTHROPIC_API_KEY` (model `claude-haiku-4-5`) **or** `GOOGLE_API_KEY` (text model `gemini-flash-lite-latest`) unlocks all text AI features (vision tagging, outfit/buy-next reasoning, trends, DresserAI chat) via `services/llm.py`; `GOOGLE_API_KEY` additionally powers TryOn image generation (`gemini-2.5-flash-image` - note: image generation needs quota/billing enabled on the Google account, separate from text). Plus `OPENWEATHER_API_KEY` (weather + geocoding), `SERPAPI_KEY`, `STRIPE_SECRET_KEY`/`STRIPE_PRICE_ID`/`STRIPE_WEBHOOK_SECRET`, `GOOGLE_OAUTH_CLIENT_ID` ("Sign in with Google": GIS button on the login page posts an ID token to `/auth/google`, verified server-side in `services/google_oauth.py`; unset hides the button), and `JWT_SECRET`. Infrastructure keys: `ENVIRONMENT` (dev/production - see Security posture), `CORS_ORIGINS` (required in production), `RATE_LIMIT_ENABLED`, `DATABASE_URL` (SQLite default; Postgres forms documented in `.env.example`), and `STORAGE_BACKEND` + `S3_*` (local default; R2/S3 for media).
